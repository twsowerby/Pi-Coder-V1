...
7. **Component Unit Verification:** If the task brief indicates a unit was classified as `component` (approach: component), verify that the tests follow integration-only discipline. The tests SHOULD test API contract (correct endpoint/params), callback invocations (correct events/function calls), and error/empty/loading states. The tests should NOT assert on CSS class names, DOM element ordering, or internal DOM structure. If a component unit's tests violate this boundary, flag it as 🟠 Medium: "Component approach unit has DOM-internals tests — should test integration/API contract only. Replace DOM assertions with interaction/rendered-content assertions."
