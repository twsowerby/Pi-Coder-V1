...
describe("Phase 1: Extension Foundation", () => {
  it("should export ORCHESTRATOR_TOOLS with the correct tool set", async () => {
    ...
    assert.deepStrictEqual(tools, [
      ...
      "pi_coder_git", "pi_coder_run_tests", "upsert_knowledge", "pi_coder_save_spec", "pi_coder_read_spec", "pi_coder_advance_fsm", "pi_coder_submit_review", "interview", "intercom",
    ]);
  });
...
// ---------------------------------------------------------------------------
// Phase 6: pi_coder_submit_review auto-transition handler
// ---------------------------------------------------------------------------

describe("Phase 6: pi_coder_submit_review auto-transition steer", () => {
  it("structured review submission triggers auto-transition steer for approved verdict", () => {
    // Simulate the rawContent mutation that the tool_result handler performs
    // when toolName === "pi_coder_submit_review" and verdict === "approved".
    // The handler appends a steer message to the first text block.
    ...
    // Construct the same steer message the handler would append
    ...
    // Simulate the rawContent mutation pattern from the handler
    const rawContent: Array<{ type: "text"; text: string }> = [
      { type: "text", text: "Review submitted: ✅ Approved. FSM advanced to APPROVED." },
    ];
    ...
    // Verify the steer was appended
    assert.ok(rawContent[0].text.includes("AUTO-TRANSITION"), "Should append AUTO-TRANSITION steer");
    assert.ok(rawContent[0].text.includes("approved"), "Steer should mention approved");
    assert.ok(rawContent[0].text.includes("APPROVED"), "Steer should mention APPROVED state");
    assert.ok(rawContent[0].text.includes("MERGING") || rawContent[0].text.includes("FINAL_APPROVAL"), "Steer should mention next action");
  });

  it("structured review submission triggers auto-transition steer for needs_changes verdict", () => {
    // TDD mode, functional fix
    const reviewDetails = {
      success: true,
      verdict: "needs_changes",
      fixType: "functional",
      issueCount: { high: 1, medium: 0, low: 0 },
      summary: "Missing null check",
    };

    let reviewSteer: string | undefined;
    if (reviewDetails.verdict === "needs_changes") {
      // TDD mode path
    ...
    const rawContent: Array<{ type: "text"; text: string }> = [
      { type: "text", text: "Review submitted: ⚠️ Needs Changes (functional fix). FSM advanced to NEEDS_CHANGES." },
    ];

    if (reviewSteer && Array.isArray(rawContent) && rawContent.length >= 1 && rawContent[0]?.type === "text") {
      rawContent[0].text += reviewSteer;
    }

    assert.ok(rawContent[0].text.includes("AUTO-TRANSITION"), "Should append AUTO-TRANSITION steer");
    assert.ok(rawContent[0].text.includes("NEEDS_CHANGES"), "Steer should mention NEEDS_CHANGES");
    assert.ok(rawContent[0].text.includes("TDD_RED_WRITE"), "Functional fix steer should mention TDD_RED_WRITE");
  });

  it("does not append steer when reviewDetails.success is false", () => {
    const reviewDetails = {
      success: false,
      verdict: undefined,
      error: "fixType_required",
    };

    // The handler checks: if (reviewDetails?.success !== false)
    // so when success is false, no steer should be appended
    const shouldAppendSteer = reviewDetails.success !== false;
