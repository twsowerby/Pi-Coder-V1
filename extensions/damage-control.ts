...
const DEFAULT_RULES: DamageControlRules = {
	bashToolPatterns: [
		...
		{ pattern: "\\bsupabase\\s+(?:db\\s+reset|migration\\s+reset)\\b", reason: "supabase db/migration reset destroys the local database — use supabase db push for incremental schema changes. If a clean DB is needed, tell the user what you need and ask them to run it manually." },
		{ pattern: "\\bsupabase\\s+db\\s+push\\s+.*--force\\b", reason: "Force-pushing schema changes overwrites remote state — use supabase db push without --force for safe incremental migrations." },
		{ pattern: "\\bsupabase\\s+(?:stop|db\\s+remove)\\b", reason: "Stopping or removing the Supabase instance destroys the local database — tell the user what you need and ask them to handle it." },
